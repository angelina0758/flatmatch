...
export function MainApp() {
  ...
  // Application tabs
  type Tab = 'discovery' | 'matrix' | 'messages' | 'owner_dashboard' | 'profile';
  ...
  const [authToken, setAuthToken] = useState<string>(() => localStorage.getItem('flatmatch_authToken') || '');

  // Keep localStorage up to date with token context
  useEffect(() => {
    if (authToken) {
      localStorage.setItem('flatmatch_authToken', authToken);
    } else {
      localStorage.removeItem('flatmatch_authToken');
    }
  }, [authToken]);
  ...
  const [schedulingListing, setSchedulingListing] = useState<Listing | null>(null);
  ...
  const [activeScheduleFilter, setActiveScheduleFilter] = useState<'all' | 'pending' | 'accepted'>('all');
  const [reschedulingActiveId, setReschedulingActiveId] = useState<string | null>(null);
  const [rescheduleTime, setRescheduleTime] = useState('15:00');
  const [selectedScheduleIdForView, setSelectedScheduleIdForView] = useState<string | null>(null);
  ...
  const [selectedListingType, setSelectedListingType] = useState<'all' | 'shared_stay' | 'entire_unit'>('all');
  ...
  const [aiBreakdown, setAiBreakdown] = useState<CompatibilityBreakdown | null>(null);
  ...
  const [newListingUtility, setNewListingUtility] = useState('Divided evenly (~₹1,500)');
  ...
  const [newListingType, setNewListingType] = useState<'shared_stay' | 'entire_unit'>('shared_stay');
  const [newListingDesc, setNewListingDesc] = useState('');
  const [newListingAmenities, setNewListingAmenities] = useState<string>('Gym, Security, Central AC, High-speed Internet');
  const [newListingGender, setNewListingGender] = useState<'Girls only' | 'Boys only' | 'No preference'>('No preference');
  const [newListingRestrictions, setNewListingRestrictions] = useState<string>('No indoor smoking, No loud parties, Respect quiet hours');
  ...
  const [editDrinking, setEditDrinking] = useState<'never' | 'socially' | 'regularly'>('socially');
  const [editSleeping, setEditSleeping] = useState<'early_bird' | 'night_owl' | 'flexible'>('flexible');
  ...
  const [reportReason, setReportReason] = useState('Romantic solicitation / dating-like behavior');
  ...
  const loadUsersDetails = async (userIdToSelect?: string) => {
    try {
      ...
      setAllUsers(data);
      if (userIdToSelect) {
        const token = authToken || localStorage.getItem('flatmatch_authToken');
        if (token) {
  ...
  const loadInitialSession = async () => {
    const listRes = await fetch('/api/v1/auth/users').then(r => r.json()).catch(() => []);
    setAllUsers(listRes);
    await fetchListings();
  ...
  const handleUserSwitch = async (userId: string, usersOverride?: User[]) => {
    ...
    try {
      ...
      setAuthToken(token);
      localStorage.setItem('flatmatch_authToken', token);
  ...
  const fetchListings = async () => {
    try {
      const typeParam = selectedListingType !== 'all' ? `type=${selectedListingType}` : '';
      const minMaxParam = `max_price=${maxPrice}`;
      const searchParam = searchQuery ? `search=${encodeURIComponent(searchQuery)}` : '';
      const geoParam = `lat=${searchLat}&lng=${searchLng}&radius=${searchRadius}`;
      
      const query = [typeParam, minMaxParam, searchParam, geoParam].filter(Boolean).join('&');
  ...
  useEffect(() => {
    fetchListings();
  }, [selectedListingType, maxPrice, searchRadius, searchLat, searchLng, searchQuery]);
  ...
  useEffect(() => {
    ...
    } else if (q.includes('bengaluru') || q.includes('bangalore') || q.includes('indiranagar')) {
      setSearchLat(12.9716);
      setSearchLng(77.5946);
    } else if (q.includes('brooklyn') || q.includes('new york') || q.includes('ny')) {
      setSearchLat(40.7128);
      setSearchLng(-74.0060);
    }
  }, [searchQuery]);
  ...
  const fetchConversations = async (tokenOverride?: string) => {
    const token = tokenOverride || authToken || localStorage.getItem('flatmatch_authToken') || '';
    if (!token) return;
    try {
  ...
  const handleProposeSchedule = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setViewingNotes('');
        fetchSchedulesOfUser(token);
        alert("Success! Your viewing appointment proposal has been recorded. An audit alert has been automatically sent to host chat channel.");
      } else {
  ...
  const handleUpdateScheduleStatus = async (bookingId: string, status: string, notes?: string, time?: string) => {
    const token = authToken || localStorage.getItem('flatmatch_authToken') || '';
    if (!token) return;
    try {
      ...
      if (res.ok) {
        ...
        alert(`Schedule status has been updated to ${status.toUpperCase()}! A confirmation receipt has been sent to Chat.`);
      } else {
        const errorData = await res.json();
  ...
  const handleSubmitVerification = async (e: React.FormEvent) => {
    ...
    if (docType === 'Aadhaar') {
      if (!/^\d{12}$/.test(licenseClean)) {
        alert('Invalid Aadhaar Card form. Must specify a 12-digit number (e.g., 9012 3456 7890).');
        return;
      }
    } else if (docType === 'PAN') {
      if (!/^[A-Z]{5}[0-9]{4}[A-Z]{1}$/i.test(licenseClean)) {
        alert('Invalid PAN format. Must be a 10-character alphanumeric string (e.g., ABCDE1234F).');
        return;
      }
    }
    ...
    try {
      ...
      const url = isSelfKyc ? '/api/v1/auth/verify-kyc' : `/api/v1/listings/${verifyingListingId}/verify`;
      ...
      if (res.ok) {
        ...
        if (isSelfKyc) {
          ...
          alert("KYC verified instantly! Your account identity is now APPROVED under UIDAI / Income Tax department audit.");
        } else {
          fetchListings();
          if (currentUser && currentUser.user_type === 'owner') {
            fetchOwnerListings();
          }
          alert("Paperwork filed! Your listing verification request is now PENDING review. Admin can instantly approve this in your Owner Bulk Hub (Approvals on Active Catalog list).");
        }
      } else {
  ...
  const handleApproveVerificationSim = async (listingId: string) => {
    ...
    if (!token) return;
    try {
      const res = await fetch(`/api/v1/listings/${listingId}/approve_verification`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      ...
      if (res.ok) {
        ...
        alert("Instant Administrative Approval Executed! Verified Badge of honor is now applied live.");
      }
    } catch (e) {
  ...
  const fetchActiveChatMessages = async (partnerId: string) => {
    ...
    if (!currentUser || !token) return;
    try {
      const res = await fetch(`/api/v1/chat/conversations/${partnerId}/messages`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    const token = authToken || localStorage.getItem('flatmatch_authToken') || '';
    if (!currentUser || !activeConversation || !newMessageText.trim() || !token) return;
    ...
    try {
      ...
      setTimeout(async () => {
        let responderText = "That sounds great! Let me review my schedule and get back to you with flatmate viewing slots.";
        if (newMessageText.toLowerCase().includes('view') || newMessageText.toLowerCase().includes('free')) {
          responderText = "I'm actually available tomorrow evening! Would you love to hop on a zoom call or check out the neighborhood layout direct?";
        } else if (newMessageText.toLowerCase().includes('pets') || newMessageText.toLowerCase().includes('dog') || newMessageText.toLowerCase().includes('cat')) {
          responderText = "I completely adore animals! There is a gorgeous garden backyard with plenty of space to run around.";
        }
  ...
  const handleCreateListing = async (e: React.FormEvent) => {
    ...
    if (!newListingRestrictions.trim()) {
      alert('At least one house restriction is mandatory. Please specify guidelines before saving.');
      return;
    }

    const pinClean = newListingPincode.trim();
    if (!newListingCity.trim() || !newListingState.trim() || !pinClean) {
      alert('Indian regulatory compliance requires mandatory City, State, and 6-digit Pincode fields.');
      return;
    }

    if (!/^[1-9][0-9]{5}$/.test(pinClean)) {
      alert('Invalid Pincode structure. Must be a 6-digit Indian Postal PIN Code not starting with 0.');
      return;
    }
    ...
    try {
      const payload = {
        ...
        description: newListingDesc || `Beautifully prepared room for FlatMatch Seekers.`,
        ...
        room_size: newListingType === 'shared_stay' ? newListingRoomSize : undefined,
        utility_split: newListingType === 'shared_stay' ? newListingUtility : undefined,
        current_flatmate_count: newListingType === 'shared_stay' ? Number(newListingFlatmates) : undefined,
        apartment_type: newListingType === 'entire_unit' ? newListingUnitType : undefined,
        gender_preference: newListingGender,
        house_restrictions: newListingRestrictions.split(',').map(item => item.trim()).filter(Boolean),
        amenities: newListingAmenities.split(',').map(item => item.trim()).filter(Boolean),
      ...
      const res = await fetch('/api/v1/listings', {
        ...
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${authToken || localStorage.getItem('flatmatch_authToken') || ''}`
        },
        body: JSON.stringify(payload),
      });
      ...
      if (!res.ok) {
        alert(rawData.error || 'Failed to submit listing. Please complete all fields.');
        return;
      }
      ...
      setNewListingRestrictions('No indoor smoking, No loud parties, Respect quiet hours');
  ...
  const handleRegisterUser = async (e: React.FormEvent) => {
    ...
    if (!/^[6-9]\d{9}$/.test(mobileWithoutCode)) {
      alert('Please provide a valid 10-digit Indian mobile number following the +91 country code.');
      return;
    }
  ...
  const handleDeleteListing = async (listingId: string) => {
    if (!confirm('Are you sure you want to deactivate and remove this property listing from the live search database?')) return;
    const token = authToken || localStorage.getItem('flatmatch_authToken') || '';
    if (!token) return;
    try {
  ...
  const handleSendReport = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        alert(data.alert || 'Your report was successfully filed with the FlatMatch Trust & Safety Team.');
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      ...
      if (bulkFormat === 'json') {
        ...
        if (lines.length < 2) {
          throw new Error("Invalid CSV format. Need header and at least 1 record row.");
        }
      ...
      if (res.ok) {
        ...
        setBulkStatus({
          type: 'error',
          message: reply.error || "Batch upload failed. Verify the syntax format correctness."
        });
      }
    } catch (e: any) {
      setBulkStatus({
        type: 'error',
        message: e.message || "Crucial error occurred parsing input file structure."
      });
    }
  };
          ...
          <form onSubmit={async (e) => {
            ...
            try {
              ...
              if (res.ok) {
                ...
                // Refresh data models
                fetchConversations(reply.token);
                fetchSchedulesOfUser(reply.token);
                fetchListings();
                ...
                if (reply.profile) {
                  ...
                  setEditProfession(reply.profile.profession || 'Professional');
                  ...
                  setEditSleeping(reply.profile.sleeping_pattern || 'flexible');
            ...
            >
              {loginLoading ? 'Negotiating Access...' : 'Authenticate Credentials'}
            </button>
          </form>
          ...
          <div className="fixed inset-0 bg-[#141414]/80 flex items-center justify-center z-50 p-4">
            <div className="w-[500px] bg-white border-4 border-[#141414] p-6 shadow-[8px_8px_0px_#101010]">
              ...
              <form onSubmit={async (e) => {
                ...
                <div>
                  <label className="block text-[9px] font-mono font-bold mb-1">FULL NAME</label>
                  <input type="text" name="full_name" required className="w-full border border-[#141414] p-2 bg-[#F5F5F3]" placeholder="e.g. Alex Johnson" />
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-[9px] font-mono font-bold mb-1">EMAIL ADDRESS</label>
                    <input type="email" name="email" required className="w-full border border-[#141414] p-2 bg-[#F5F5F3]" placeholder="e.g. seeker@example.com" />
                  </div>
                  <div>
                    <label className="block text-[9px] font-mono font-bold mb-1">SECURE PASSWORD</label>
                    <input type="password" name="password" required className="w-full border border-[#141414] p-2 bg-[#F5F5F3]" placeholder="password123" defaultValue="password123" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                  ...
                  <div>
                    <label className="block text-[9px] font-mono font-bold mb-1">INDIAN MOBILE (MUST START +91)</label>
                    <input type="text" name="phone_number" required className="w-full border border-[#141414] p-2 bg-[#F5F5F3]" placeholder="+919876543210" defaultValue="+919876543210" />
                  </div>
                </div>
                <button type="submit" className="w-full bg-[#D2F57C] hover:bg-[#b8da64] border-2 border-[#141414] font-bold text-center py-2.5 uppercase text-xs">
      ...
      <div className="w-[1024px] h-[768px] bg-[#E4E3E0] text-[#141414] font-sans overflow-hidden flex flex-col justify-center items-center border-[8px] border-[#141414] mx-auto relative shadow-2xl">
        <div className="w-[480px] bg-white border-4 border-[#141414] p-8 shadow-[8px_8px_0px_#141414] text-center">
          ...
          <p className="text-xs font-mono text-neutral-600 mt-4 leading-relaxed bg-[#FAF9F5] p-3 border border-neutral-300">
            Unauthorized profile structure category. The requested route sequence is strictly reserved for authenticated profiles utilizing distinct dashboard views. Seekers & tenants cannot explore corporate listing hub assets.
          </p>
  ...
  return (
    <div className="w-[1024px] h-[768px] bg-[#E4E3E0] text-[#141414] font-sans overflow-hidden flex flex-col border-[8px] border-[#141414] mx-auto relative shadow-2xl">
      ...
      <header className="h-12 border-b border-[#141414] flex items-center justify-between px-4 bg-white shrink-0 z-20">
        ...
        <div className="flex items-center gap-3 border-l border-[#141414] pl-4 h-full py-1">
          <div className="text-right">
            <div className="text-[9px] font-mono opacity-50 uppercase tracking-widest leading-none">Simulation Role</div>
            <select 
        ...
        {activeTab === 'discovery' && (
          <div className="flex-1 flex overflow-hidden">
            ...
            <aside className="w-[310px] border-r border-[#141414] flex flex-col bg-white shrink-0">
              ...
              <div className="p-4 border-b border-[#141414] bg-[#F5F5F3]">
                <div className="flex justify-between items-end mb-2">
                  <h2 className="font-serif italic text-xs font-bold uppercase tracking-wide">Discovery Filter</h2>
                ...
                {/* Active Zone Buttons */}
                <div className="mb-2">
                  <div className="text-[9px] font-mono mb-1 text-zinc-500 uppercase tracking-wider">Regions & Metro Hubs</div>
                  <div className="flex flex-wrap gap-1">
                    {[
                      ...
                      const isSelected = Math.abs(searchLat - city.lat) < 0.2 && Math.abs(searchLng - city.lng) < 0.2;
                      return (
                        <button
                          key={city.name}
                          type="button"
                          onClick={() => {
                            setSearchQuery(city.name === "New York" ? "New York" : city.name);
                          ...
                          className={`text-[8.5px] px-1.5 py-0.5 border border-[#141414] font-mono font-bold leading-normal uppercase transition-all ${
                            isSelected ? "bg-[#D2F57C] text-black" : "bg-white hover:bg-zinc-100 text-zinc-800"
                          }`}
                        >
                          {city.name}
                        </button>
                ...
                <div className="relative mb-2">
                  ...
                  {searchQuery && (
                    <button onClick={() => setSearchQuery('')} className="absolute right-2.5 top-3 text-xs opacity-60 hover:opacity-100">
                ...
                <div className="space-y-2 pt-1">
                  <div>
                    <div className="flex justify-between text-[9px] font-mono mb-1">
                      ...
                      <span className="font-bold uppercase tracking-wider">{selectedListingType.replace('_', ' ')}</span>
                    </div>
                  ...
                  <div>
                    <div className="flex justify-between text-[9px] font-mono mb-0.5">
                      ...
                      <span className="font-bold">{formatPrice(maxPrice)}/mo</span>
                    </div>
                    <input 
                      type="range" 
                      min="5000" 
              ...
              <div className="flex-1 overflow-y-auto divide-y divide-[#141414]">
                {listings.length === 0 ? (
                  <div className="p-6 text-center text-xs opacity-60 font-mono italic">
                    No active properties found matching filters.<br/>Try adjusting pricing limits or searching "Bengaluru".
                  </div>
                ) : (
                  listings.map((l, idx) => {
                      ...
                      >
                        ...
                        <div className="pr-1 overflow-hidden">
                          <div className="text-xs font-black leading-snug truncate flex items-center gap-1">
                            {l.is_verified && <ShieldCheck className="w-3.5 h-3.5 text-green-600 shrink-0 inline" title="FlatMatch Verified Stay" />}
                            <span className="truncate">{l.title}</span>
                          </div>
                          <div className="text-[9px] opacity-70 mt-0.5 font-mono truncate">{l.address}</div>
                          <div className="flex flex-wrap gap-1 mt-1.5 items-center">
                            <span className={`text-[7.5px] font-extrabold px-1.5 border leading-none py-0.5 tracking-tighter uppercase rounded-none ${
                              l.listing_type === 'shared_stay' 
                                ? (isSelected ? 'bg-[#FFFBEB] text-[#78350F] border-[#FCD34D]' : 'bg-amber-100 text-amber-900 border-amber-300')
                                : (isSelected ? 'bg-[#EFF6FF] text-[#1E3A8A] border-[#93C5FD]' : 'bg-blue-100 text-blue-900 border-blue-300')
                            }`}>
                              {l.listing_type === 'shared_stay' ? '🤝 ROOMMATE MATCH' : '🏢 LANDLORD UNIT'}
                            </span>
                            {l.room_size && (
                              <span className="text-[7.5px] font-mono opacity-85 underline decoration-dotted">{l.room_size}</span>
                            )}
                          </div>
                        </div>
                        <div className="text-right flex flex-col justify-between items-end">
                          <div className="font-mono text-xs font-bold">{formatPrice(l.price_per_month)}</div>
            ...
            <main className="flex-1 flex flex-col overflow-y-auto p-4 gap-4">
              {selectedListing ? (
                <div className="grid grid-cols-[1fr_310px] gap-4" style={{ height: '100%', minHeight: '680px' }}>
                  ...
                  <div className="flex flex-col gap-4 overflow-y-auto">
                    <section className="border-2 border-[#141414] bg-white p-5 shadow-[4px_4px_0px_#141414] flex flex-col justify-between">
                      <div>
                          ...
                          <h1 className="text-xl font-black uppercase tracking-tight leading-dense flex flex-wrap items-center gap-1.5">
                            ...
                            {selectedListing.is_verified && (
                              <span className="inline-flex items-center gap-1 bg-[#D2F57C] text-black border border-[#141414] px-1.5 py-0.5 text-[8.5px] font-extrabold tracking-wider uppercase shrink-0 shadow-[1px_1px_0px_#141414] animate-pulse">
                                <ShieldCheck className="w-3.5 h-3.5 text-green-700" /> Verified Stay
                              </span>
                            )}
                          </h1>
                          <div className="px-2 py-0.5 bg-[#141414] text-white text-[9px] font-mono uppercase border border-[#141414] font-bold shrink-0">
                            {selectedListing.listing_type === 'shared_stay' ? 'Shared Stay' : 'Entire Unit'}
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 border-t border-[#141414] pt-3">
                          <div className="space-y-2">
                            <div>
                              <div className="text-[8.5px] font-bold uppercase opacity-50 mb-0.5 tracking-wider font-mono">Location Coordinates</div>
                            ...
                            {selectedListing.listing_type === 'shared_stay' ? (
                              <>
                                <div>
                                  <div className="text-[8.5px] font-bold uppercase opacity-50 mb-0.5 tracking-wider font-mono">Private Room Metrics</div>
                                  <div className="text-xs font-mono font-bold bg-[#F5F5F3] px-2 py-0.5 border border-[#141414] inline-block">{selectedListing.room_size || "12' x 14' Size"}</div>
                                </div>
                                <div>
                                  <div className="text-[8.5px] font-bold uppercase opacity-50 mb-0.5 tracking-wider font-mono">Utilities splitting rules</div>
                                  <div className="text-xs font-mono">{selectedListing.utility_split || "Equally split between roommates (~$65)"}</div>
                                </div>
                              </>
                            ) : (
                              <>
                                <div>
                                  <div className="text-[8.5px] font-bold uppercase opacity-50 mb-0.5 tracking-wider font-mono">Apartment Type</div>
                                  <div className="text-xs font-bold">{selectedListing.apartment_type || 'Studio Flat'}</div>
                                </div>
                                <div>
                                  <div className="text-[8.5px] font-bold uppercase opacity-50 mb-0.5 tracking-wider font-mono">Lease Security Deposit</div>
                                  <div className="text-xs font-bold text-red-700">{formatPrice(selectedListing.deposit)}</div>
                                </div>
                              </>
                            )}
                          </div>
                          ...
                          <div className="space-y-2">
                            <div>
                              <div className="text-[8.5px] font-bold uppercase opacity-50 mb-0.5 tracking-wider font-mono font-black">Monthly Pricing</div>
                              <div className="text-lg font-mono font-black text-green-700 leading-none">{formatPrice(selectedListing.price_per_month)}<span className="text-[10px] font-bold ml-1 text-black">/month</span></div>
                            </div>

                            <div>
                              <div className="text-[8.5px] font-bold uppercase opacity-50 mb-0.5 tracking-wider font-mono">Listing Host / Owner</div>
                              <div className="text-xs font-black">{selectedListing.owner_name || 'Marcus Sterling Properties'}</div>
                            </div>

                            <div>
                              <div className="text-[8.5px] font-bold uppercase opacity-50 mb-0.5 tracking-wider font-mono">Included Amenities</div>
                              <div className="flex flex-wrap gap-1 mt-1">
                                {selectedListing.amenities.map((a, i) => (
                                  <span key={i} className="px-1.5 py-0.5 bg-[#D2F57C] text-[#141414] text-[8px] font-black border border-[#141414] uppercase leading-none">
                                    {a}
                                  </span>
                                ))}
                              </div>
                            ...
                            <div>
                              <div className="text-[8.5px] font-bold uppercase opacity-50 mb-1 tracking-wider font-mono text-red-700 font-extrabold">Roommate Gender Preference</div>
                              <div className="text-xs font-bold font-mono text-red-950 bg-red-50/70 border-2 border-red-400 px-2.5 py-1 inline-block uppercase tracking-tight">
                                🚻 {selectedListing.gender_preference || "No preference"}
                              </div>
                            </div>

                            <div>
                              <div className="text-[8.5px] font-bold uppercase opacity-50 mb-1 tracking-wider font-mono text-red-700 font-extrabold">Mandatory House Rules</div>
                              <div className="flex flex-wrap gap-1.5 mt-1">
                                {(selectedListing.house_restrictions || ["No smoking indoors", "Respect quiet hours", "No commercial parties"]).map((r, i) => (
                                  <span key={i} className="px-2 py-0.5 bg-red-50 text-red-900 border border-red-300 text-[8px] font-black uppercase leading-none rounded-none flex items-center gap-1 shadow-[1px_1px_0px_#ef4444]">
                                    ⛔ {r}
                                  </span>
                                ))}
                              </div>
                      ...
                      <div className="mt-4 pt-3 border-t border-dashed border-zinc-400 flex flex-wrap justify-between items-center gap-2">
                        {currentUser?.id !== selectedListing.owner_id ? (
                          <>
                            <button 
                              onClick={() => handleInitiateChat(selectedListing.owner_id)}
                            ...
                            <button 
                              onClick={() => {
                                alert(`Simulating Application Request registered for host: ${selectedListing.owner_name}. Verification is now waiting on tenant response.`);
                              }}
                          ...
                          <div className="flex justify-between w-full items-center">
                            <span className="text-[10px] font-mono text-zinc-500">This is your property listing.</span>
                            <button 
                    ...
                    <section className="border-2 border-[#141414] bg-white p-3 shadow-[4px_4px_0px_#141414] flex-1 flex flex-col justify-between">
                      <div className="text-[10px] font-mono uppercase opacity-50 mb-2">Unit Imagery Media (Max 10 High-res Uploads)</div>
                      <div className="grid grid-cols-2 gap-2 h-[210px]">
                        {selectedListing.image_urls && selectedListing.image_urls.length > 0 ? (
                          selectedListing.image_urls.map((img, i) => (
                            <div key={i} className="border border-[#141414] overflow-hidden relative group bg-neutral-100">
                              <img 
                                src={img} 
                        ...
                        {selectedListing.image_urls && selectedListing.image_urls.length === 1 && (
                          <div className="border border-dashed border-[#141414] flex flex-col items-center justify-center text-zinc-500 bg-[#F5F5F3] p-4 text-center">
                            ...
                            <span className="text-[9px] font-mono font-bold uppercase tracking-wider">Drag & drop photos</span>
                            <span className="text-[8px] opacity-60">to fill slot vacancy</span>
                          </div>
                        )}
                      </div>
                    </section>
                  ...
                  <div className="flex flex-col gap-4">
                    <section className="border-2 border-[#141414] bg-white p-4 shadow-[4px_4px_0px_#141414] flex-1 flex flex-col relative h-[380px] overflow-hidden">
                      ...
                      <div className="flex-1 bg-[#EEEDE9] relative border border-[#141414] overflow-hidden">
                        <div className="absolute inset-0 opacity-[0.06] pointer-events-none" style={{ backgroundImage: 'radial-gradient(#141414 1.5px, transparent 1.5px)', backgroundSize: '16px 16px' }}></div>
                        ...
                        >
                          <span className="text-[8px] font-mono opacity-50 uppercase absolute bottom-2">Radius Limit ({searchRadius} km)</span>
                        </div>
                        ...
                        {listings.map((l) => {
                          ...
                          const dLng = (l.longitude - searchLng) * scaleFactor * cosLat;
                          ...
                          return (
                            ...
                            >
                              <div className="relative flex flex-col items-center">
                                <div className={`font-mono text-[8px] py-0.5 px-1 font-bold ${isCurrent ? 'bg-[#141414] text-white border border-[#141414]' : 'bg-white text-zinc-800 border border-[#141414]'} leading-none rounded-sm mb-0.5 whitespace-nowrap shadow-sm`}>
                                  {formatPrice(l.price_per_month)}
                                </div>
                                <div className={`w-3.5 h-3.5 rounded-full flex items-center justify-center border ${isCurrent ? 'bg-[#D2F57C] border-[#141414] animate-pulse ring-4 ring-[#D2F57C]/40' : 'bg-white border-[#141414]'}`}>
                    ...
                    <section className="border-2 border-[#141414] bg-white p-3.5 shadow-[4px_4px_0px_#141414] h-[200px] flex flex-col justify-between">
                      <div>
                        ...
                        <div className="space-y-2 mt-2">
                           <div className="flex items-start gap-2">
                             ...
                             <div>
                               <div className="text-[10px] font-bold">KYC Identity Check: {currentUser?.is_verified ? 'Approved & Sealed' : 'Pending Verification'}</div>
                               <p className="text-[9px] opacity-60">Verified government issued Aadhaar or PAN card seals trust across FlatMatch India.</p>
                               {!currentUser?.is_verified && (
                                 <button
                                   type="button"
                                   onClick={() => {
                          ...
                          <div className="flex items-start gap-2">
                            <Check className="w-4 h-4 text-green-600 bg-green-100 border border-green-500 p-0.5 shrink-0" />
                            <div>
                              <div className="text-[10px] font-bold">In-App Chat Protection: Enabled</div>
                              <p className="text-[9px] opacity-60">Personal emails and private mobile details are securely hidden inside our WebSocket server.</p>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-[#F5F5F3] p-1.5 border border-[#141414] text-[9px] font-mono flex justify-between items-center">
                        <span className="font-bold">Subscription Access Tier:</span>
                ...
                <div className="border-2 border-dashed border-[#141414] bg-white p-12 text-center my-auto flex flex-col items-center justify-center">
                  ...
                  <div className="text-sm font-bold uppercase tracking-wider mb-1">No Listing Selected</div>
                  <p className="text-xs opacity-60 max-w-sm">
                    Toggle discovery preferences on side panel index to query available stays currently available across FlatMatch.
                  </p>
                </div>
              )}
            </main>
        ...
        {activeTab === 'matrix' && (
          <div className="flex-1 flex overflow-hidden">
            ...
            <aside className="w-[300px] border-r border-[#141414] flex flex-col bg-white shrink-0">
              <div className="p-4 border-b border-[#141414] bg-[#F5F5F3]">
                <h2 className="font-serif italic text-xs font-bold uppercase tracking-wide mb-1">Roomie Match Partners</h2>
              ...
              <div className="flex-1 overflow-y-auto divide-y divide-[#141414]">
                {matches.length === 0 ? (
                  <div className="p-6 text-center text-xs opacity-60 font-mono italic">
                    No matching profiles found.<br />Ensure your user profile simulation matches seekers with tenants. Change role in header above.
                  </div>
                ) : (
                  matches.map((m, idx) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex justify-between items-start mb-1">
                          <div>
                            <div className="text-xs font-black leading-tight">{m.user.full_name}</div>
                            <div className="text-[9px] opacity-75 font-mono">{m.profile.profession} // Age {m.profile.age}</div>
                          </div>
                          <div className={`font-mono text-sm font-black px-1.5 py-0.5 border ${isSelected ? 'bg-[#D2F57C] text-[#141414] border-[#D2F57C]' : 'bg-[#141414] text-[#D2F57C] border-[#111]'}`}>
                            {m.score}%
                          </div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-1 mt-1.5">
                          <span className={`text-[8px] font-mono px-1 border uppercase ${isSelected ? 'border-zinc-500' : 'border-zinc-300 bg-zinc-50'}`}>
            ...
            <main className="flex-1 overflow-y-auto p-5 bg-[#E4E3E0] flex flex-col gap-4">
              {selectedMatch ? (
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <section className="border-2 border-[#141414] bg-white p-5 shadow-[4px_4px_0px_#141414]">
                      <div className="flex justify-between items-start mb-4 pb-2 border-b border-[#141414]">
                        <div>
                          <div className="text-[10px] font-mono opacity-50 uppercase tracking-widest leading-none mb-1">Direct Match Insight</div>
                          <h2 className="text-lg font-black tracking-tight">{currentUser?.full_name} ⇆ {selectedMatch.user.full_name}</h2>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="p-3 border border-[#141414] bg-[#F5F5F3]">
                          <div className="text-[9px] font-mono opacity-60 uppercase mb-1 font-bold">Your Preference Profile</div>
                          <div className="space-y-1 text-xs">
                            <div><span className="opacity-50">Tidiness Level:</span> <strong>{currentProfile?.cleanliness_level}/5</strong></div>
                            <div><span className="opacity-50">Pet Policy:</span> <strong>{currentProfile?.pets_allowed ? 'Allowed' : 'Prohibited'}</strong></div>
                            <div><span className="opacity-50">Smoking:</span> <strong>{currentProfile?.smoker ? 'Smokes' : 'No smoke'}</strong></div>
                            <div><span className="opacity-50">Budget Limit:</span> <strong>{currentProfile ? `${formatPrice(currentProfile.budget_min)} - ${formatPrice(currentProfile.budget_max)}` : ''}</strong></div>
                            <div><span className="opacity-50">Workspace Status:</span> <strong className="uppercase">{currentProfile?.wfh_status}</strong></div>
                          </div>
                        </div>

                        <div className="p-3 border border-[#141414] bg-[#D2F57C]/10">
                          <div className="text-[9px] font-mono text-[#141414]/70 uppercase mb-1 font-bold">Their Preference Profile</div>
                          <div className="space-y-1 text-xs">
                            <div><span className="opacity-50">Tidiness Level:</span> <strong>{selectedMatch.profile.cleanliness_level}/5</strong></div>
                            <div><span className="opacity-50">Pet Policy:</span> <strong>{selectedMatch.profile.pets_allowed ? 'Allowed' : 'Prohibited'}</strong></div>
                            <div><span className="opacity-50">Smoking:</span> <strong>{selectedMatch.profile.smoker ? 'Smokes' : 'No smoke'}</strong></div>
                            <div><span className="opacity-50">Budget Limit:</span> <strong>{`${formatPrice(selectedMatch.profile.budget_min)} - ${formatPrice(selectedMatch.profile.budget_max)}`}</strong></div>
                            <div><span className="opacity-50">Workspace Status:</span> <strong className="uppercase">{selectedMatch.profile.wfh_status}</strong></div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mt-4 p-3.5 border border-[#141414] bg-white">
                        <div className="text-[9px] font-mono uppercase opacity-50 mb-1">Partner Personal Statement // Biography</div>
                        <p className="text-xs font-serif leading-relaxed italic text-zinc-700">
                          "{selectedMatch.profile.bio || "No custom greeting bio updated on FlatMatch profile listings."}"
                        </p>
                      </div>

                      {/* QUICK DESTRUCT CHAT CALL */}
                      <button 
                        onClick={() => handleInitiateChat(selectedMatch.user.id)}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <section className="border-2 border-[#141414] bg-white p-4 shadow-[4px_4px_0px_#141414]">
                        ...
                        <ul className="space-y-1.5">
                          {selectedMatch.matchFactors.map((f: string, i: number) => (
                            <li key={i} className="text-xs flex items-start gap-1.5 font-bold">
                              <span className="text-green-600 font-mono">✔</span>
                              <span className="text-stone-700">{f}</span>
                            </li>
                          ))}
                          {selectedMatch.matchFactors.length === 0 && (
                            <li className="text-[11px] opacity-60 italic font-mono">No direct alignment filters catalogued.</li>
                          )}
                        </ul>
                      </section>
                      ...
                      <section className="border-2 border-[#141414] bg-white p-4 shadow-[4px_4px_0px_#141414]">
                        ...
                        <ul className="space-y-1.5">
                          {selectedMatch.mismatchFactors.map((f: string, i: number) => (
                            <li key={i} className="text-xs flex items-start gap-1.5 font-bold text-stone-850">
                      ...
                      <div>
                        ...
                        {loadingAi ? (
                          ...
                          <div className="text-[11px] leading-relaxed text-zinc-800 space-y-2 max-h-[360px] overflow-y-auto pr-1">
                            ...
                            {aiBreakdown.aiAnalysis.split('\n\n').map((para, idx) => {
                              if (para.startsWith('###')) {
                                return <h4 key={idx} className="font-extrabold uppercase text-[10px] tracking-wider text-indigo-950 mt-2">{para.replace('###', '').trim()}</h4>;
                              }
                              if (para.startsWith('*')) {
                                return (
                                  <ul key={idx} className="list-disc pl-4 space-y-1 text-[10.5px]">
                                    {para.split('\n').map((li, lidx) => (
                              ...
                              return <p key={idx} className="font-serif italic text-stone-700">{para}</p>;
                            })}
                          </div>
                        ) : (
                          <div className="text-center py-12 opacity-60 text-xs italic font-mono">
                            Ensure a valid GEMINI_API_KEY is configured in your secrets page.
                          </div>
                        )}
                      </div>
                      
                      <div className="bg-amber-50 p-2.5 border border-amber-400 mt-4 rounded-none text-[8.5px] font-serif leading-snug">
                        💡 <strong>Simulation Tips:</strong> Try switching profiles from alex to dave in the header drop-down to see custom compatibility scoring and real-time AI roommate reports with different lifestyle matches.
                      </div>
                    </section>
                  </div>
                ...
                <div className="border-2 border-dashed border-[#141414] bg-white p-12 text-center my-auto">
                  Select a flatmate partner from list stream to run algorithmic compatibility review.
                </div>
              )}
            </main>
          </div>
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            <aside className="w-[280px] border-r border-[#141414] flex flex-col bg-white shrink-0 overflow-y-auto">
              ...
              <div className="p-3 border-b border-[#141414] bg-[#F5F5F3] flex justify-between items-center text-xs font-black uppercase tracking-wider">
                <span className="flex items-center gap-1"><MessageSquare className="w-3.5 h-3.5 text-[#141414]" /> Dialogues Inbox</span>
                <span className="text-[10px] bg-[#141414] text-white px-1.5 py-0.2 font-mono">{conversations.length}</span>
              </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex justify-between items-center mb-0.5">
                          <span className="text-[11px] font-black truncate pr-1">{c.participant.full_name}</span>
                          <span className="text-[7.5px] font-mono opacity-50 shrink-0 uppercase">
                            {c.participant.user_type}
                          </span>
                        </div>
                        {c.lastMessage && (
                          <p className="text-[9.5px] opacity-75 truncate">{c.lastMessage.message_text}</p>
                        )}
                      </div>
                    );
                  })
              ...
              {/* SECTION B: SCHEDULES */}
              <div className="p-3 border-b border-[#141414] bg-[#F5F5F3] flex justify-between items-center text-xs font-black uppercase tracking-wider">
                <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5 text-indigo-700" /> Viewing Tours</span>
                <span className="text-[10px] bg-indigo-700 text-white px-1.5 py-0.2 font-mono">{allSchedules.length}</span>
              </div>

              <div className="flex-1 overflow-y-auto divide-y divide-[#141414]">
                {allSchedules.length === 0 ? (
                  <div className="p-6 text-center text-[10px] opacity-60 font-mono italic">
                    No space tours scheduled.<br/>Propose an alternate hour in listings today.
                  </div>
                ) : (
                  allSchedules.map((s) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <div className="text-[10.5px] font-black truncate max-w-[170px]">{s.listing_title}</div>
                          <span className={`text-[7.5px] px-1 border uppercase font-mono shrink-0 ${
                            s.status === 'accepted' ? 'bg-[#D2F57C] text-black border-[#D2F57C]' :
                            s.status === 'declined' ? 'bg-red-100 text-red-700 border-red-350' : 'bg-amber-100 text-amber-700 border-amber-300'
                          }`}>
                            {s.status === 'accepted' ? 'Approved' : s.status === 'declined' ? 'Declined' : 'Proposed'}
                          </span>
                        </div>
                        <div className="text-[9.5px] font-mono opacity-80 flex items-center gap-1">
                          <Clock className="w-3 h-3 shrink-0" /> {bTime.toLocaleDateString()} {bTime.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                        </div>
                        <div className="text-[8.5px] opacity-60 truncate">With {currentUser?.id === s.host_id ? s.seeker_name : s.host_name}</div>
                      </div>
                    );
                  })
                )}
            ...
            <main className="flex-1 flex flex-col bg-[#E4E3E0] p-4 min-w-0">
              ...
              {activeConversation && (
                <div className="flex-1 border-2 border-[#141414] bg-white shadow-[4px_4px_0px_#141414] flex flex-col overflow-hidden">
                  ...
                  <div className="p-3 border-b border-[#141414] bg-[#F5F5F3] flex justify-between items-center">
                    <div>
                      <div className="text-xs font-black">{activeConversation.participant.full_name}</div>
                      <div className="text-[9px] font-mono opacity-50 uppercase tracking-widest">
                        {activeConversation.participant.email} • {activeConversation.participant.user_type} channel
                      </div>
                    </div>
                    {/* Mini avatar & Report action */}
                    <div className="flex items-center gap-2">
                      <button 
                        onClick={() => handleShowReportUserModal(activeConversation.participant)}
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-3.5">
                    ...
                    <div className="p-3 bg-red-50 border-2 border-red-400 text-[10.5px] text-red-910 leading-normal flex gap-2.5 shadow-[2px_2px_0px_rgba(220,38,38,0.1)]">
                      ...
                      <div>
                        <span className="font-extrabold uppercase text-[9px] font-mono tracking-wider block mb-0.5 text-red-800">FlatMatch Platonic Guarantee Policy:</span>
                        This is strictly a housing, rent-share, and roommate matching network. **FlatMatch strictly forbids any romantic solicitation, flirting, or pickup lines.** Dating behavior will result in severe penalty and permanent blacklisting. Press <strong className="font-black text-red-950">"⚠️ Report Dating Behavior"</strong> above to flag inappropriate conversations instantly.
                      </div>
                    </div>
                    {chatMessages.length === 0 ? (
                      <div className="p-6 text-center text-xs text-zinc-500 font-mono italic">
                        No previous dialogue recorded. Write an introductory greeting below.
                      </div>
                    ) : (
                      chatMessages.map((m) => {
                        ...
                        return (
                          <div key={m.id} className={`flex gap-2.5 ${isMe ? 'justify-end' : 'justify-start'}`}>
                            ...
                            <div className="max-w-[70%] text-wrap break-all">
                              <div className={`p-3.5 text-xs border border-[#141414] relative whitespace-pre-wrap ${isMe ? 'bg-[#D2F57C] text-[#141414]' : 'bg-[#F5F5F3] text-[#141414]'}`}>
                                {m.message_text}
                                {m.media_url && (
                                  <div className="mt-2 border border-[#141414] max-h-40 overflow-hidden bg-white">
                                    <img src={m.media_url} alt="Shared document or property preview" className="w-full object-contain" referrerPolicy="no-referrer" />
                                  </div>
                                )}
                              </div>
                              <div className={`text-[8px] font-mono opacity-50 mt-1 ${isMe ? 'text-right' : 'text-left'}`}>
                  ...
                  <form onSubmit={handleSendMessage} className="p-3 border-t border-[#141414] bg-white flex gap-2">
                    <div className="flex-1 relative flex flex-col gap-1">
                      ...
                      <div className="flex gap-1.5 items-center">
                        <span className="text-[8.5px] font-mono text-zinc-500 uppercase font-bold">Attach image:</span>
                        <input 
                          type="text" 
                        ...
                        />
                        {attachedMediaUrl && (
                          <button type="button" onClick={() => setAttachedMediaUrl('')} className="text-red-500 text-[9px] font-bold">Clear</button>
                        )}
                      </div>
                    </div>
                    <button 
              ...
              {selectedScheduleIdForView && (
                (() => {
                  const booking = allSchedules.find(as => as.id === selectedScheduleIdForView);
                  if (!booking) return <div className="p-6 bg-white border-2 border-[#141414]">Viewing schedule not found.</div>;
                  ...
                  return (
                    <div className="flex-1 border-2 border-[#141414] bg-white shadow-[4px_4px_0px_#141414] p-5 flex flex-col h-full overflow-y-auto">
                      ...
                      <div className="pb-3 border-b border-[#141414] mb-3 flex justify-between items-start">
                        <div>
                          <span className="text-[9.5px] font-mono opacity-50 uppercase tracking-widest block font-bold">FlatMatch Tour Apppointments</span>
                          <h2 className="text-sm font-black uppercase tracking-tight">{booking.listing_title}</h2>
                          <div className="text-[10px] font-bold text-stone-600 flex items-center gap-1 mt-0.5">
                            <MapPin className="w-3 h-3 text-red-500" /> {booking.listing_address}
                          </div>
                        </div>
                        <div className="text-right">
                          <span className="text-[8px] font-mono opacity-50 block font-bold">BOOKING CODE</span>
                          <span className="text-[9.5px] font-mono font-bold bg-[#141414] text-[#D2F57C] px-1.5 py-0.5 border border-[#141414]">{booking.id}</span>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-3">
                        <div className="p-2.5 bg-[#F5F5F3] border border-[#141414] text-[10.5px]">
                          <span className="text-[8px] font-mono opacity-60 uppercase block font-bold mb-1">Participants Matrix</span>
                          <div className="space-y-1">
                            <div><span className="opacity-50 font-mono">Host:</span> <strong>{booking.host_name}</strong> {isHost && <span className="text-[8px] bg-[#141414] text-white px-1 ml-1 font-mono uppercase">You</span>}</div>
                            <div><span className="opacity-50 font-mono">Room Seeker:</span> <strong>{booking.seeker_name}</strong> {!isHost && <span className="text-[8px] bg-[#141414] text-white px-1 ml-1 font-mono uppercase">You</span>}</div>
                            <div><span className="opacity-50 font-mono font-bold">Proposed:</span> <strong className="text-indigo-700">{bookingDate.toLocaleString()}</strong></div>
                          </div>
                        </div>

                        <div className="p-2.5 bg-[#F5F5F3] border border-[#141414] flex flex-col justify-between">
                          <div>
                            <span className="text-[8px] font-mono opacity-60 block font-bold uppercase mb-0.5">Approval Status</span>
                            <span className={`inline-block text-[10px] font-black uppercase tracking-wide border px-2 py-0.5 mt-0.5 border-[#141414] ${
                              booking.status === 'accepted' ? 'bg-[#D2F57C] text-black' : 
                              booking.status === 'declined' ? 'bg-red-100 text-red-700' :
                              booking.status === 'proposed_by_seeker' ? 'bg-amber-100 text-amber-700' : 'bg-purple-100 text-purple-700'
                            }`}>
                              {booking.status === 'accepted' ? 'Approved ✅' :
                               booking.status === 'declined' ? 'Declined ❌' :
                               booking.status === 'proposed_by_seeker' ? 'Pending Host Approval ⏳' : 'Rescheduled 🔄'}
                            </span>
                          </div>
                          <div className="text-[8px] font-mono opacity-50 uppercase mt-1">Ref hour: {new Date(booking.created_at).toLocaleDateString()}</div>
                        </div>
                      </div>

                      <div className="mb-3">
                        <label className="block text-[9.5px] font-bold uppercase tracking-wider mb-1 font-mono">Audit Booking specifications</label>
                        <div className="w-full border border-[#141414] bg-neutral-50 p-2.5 text-xs font-serif leading-relaxed italic text-zinc-700">
                          "{booking.notes || 'No specialized viewing requirements submitted.'}"
                        </div>
                      </div>

                      {/* Interactive update actions */}
                      <div className="bg-amber-50/10 border border-[#141414] p-3 mb-3">
                        <span className="text-[9.5px] font-mono uppercase font-black block mb-2 text-zinc-700">Actions Panel / Response Controls</span>
                        
                        <div className="flex flex-wrap gap-1.5">
                          {booking.status === 'proposed_by_seeker' && isHost && (
                            <>
                              <button 
                                onClick={() => handleUpdateScheduleStatus(booking.id, 'accepted', 'Host has approved and scheduled viewing!')}
                              ...
                              <button 
                                onClick={() => handleUpdateScheduleStatus(booking.id, 'declined', 'Host has declined this slot proposal.')}
                          ...
                          {booking.status === 'proposed_by_host' && !isHost && (
                            <>
                              <button 
                                onClick={() => handleUpdateScheduleStatus(booking.id, 'accepted', 'Seeker approved rescheduled counter proposal!')}
                                className="bg-[#D2F57C] text-black hover:bg-[#c4e66d] font-black uppercase text-[9.5px] px-3 py-1.5 border border-[#141414] cursor-pointer shadow-[1px_1px_0px_#141414]"
                              >
                                Accept Reschedule
                              </button>
                              <button 
                                onClick={() => handleUpdateScheduleStatus(booking.id, 'declined', 'Seeker declined rescheduled proposal.')}
                                className="bg-red-50 text-red-700 hover:bg-red-100 font-bold uppercase text-[9.5px] px-3 py-1.5 border border-[#141414] cursor-pointer"
                              >
                                Decline
                              </button>
                          ...
                          {booking.status !== 'declined' && (
                            <div className="w-full mt-2 pt-2 border-t border-[#141414] border-dashed">
                              <span className="text-[8.5px] font-mono uppercase block font-black mb-1">🔄 Propose Alternative Viewing Slot Time</span>
                              <div className="flex gap-1.5 items-center">
                                <input 
                                  type="datetime-local"
                                ...
                                <button 
                                  onClick={() => handleUpdateScheduleStatus(
                                    booking.id, 
                                    isHost ? 'proposed_by_host' : 'proposed_by_seeker', 
                                    isHost ? 'Rescheduled alternative suggested by Host.' : 'Alternative schedule proposed by Seeker.',
                                    rescheduleTime
                                  )}
                      ...
                      <div className="border border-[#141414] bg-[#F5F5F3] p-3">
                        <span className="text-[9.5px] font-mono uppercase font-black block mb-1 text-stone-850 flex items-center gap-1">
                          <Calendar className="w-3.5 h-3.5 text-indigo-700 animate-pulse" /> Real Calendar Sync Integration
                        </span>
                        <p className="text-[9px] text-zinc-600 mb-2.5">
                          Synchronize to your real external work calendars (GCal, Apple, Outlook) to enable tracking alerts.
                        </p>
                        <div className="flex flex-wrap gap-1.5">
                          <a 
              ...
              {!activeConversation && !selectedScheduleIdForView && (
                <div className="border-2 border-dashed border-[#141414] bg-white p-12 text-center my-auto flex flex-col items-center justify-center">
                  ...
                  <div className="text-sm font-bold uppercase tracking-wider mb-1">Select Chat Inbox or Viewing</div>
                  <p className="text-xs opacity-60 max-w-sm">
                    Select an active conversation channel or open a Viewing space tour from the sidebar index list on the left.
                  </p>
                </div>
              )}
            </main>
            ...
            {showBulkImporter && (
              <aside className="w-[330px] border-r border-[#141414] flex flex-col bg-white shrink-0 p-4 justify-between overflow-y-auto">
                <div>
                  ...
                  {bulkStatus && (
                    <div className={`mt-3 p-2.5 border text-[9px] font-mono whitespace-pre-wrap ${bulkStatus.type === 'success' ? 'bg-green-50 border-green-400 text-green-800' : 'bg-red-50 border-red-400 text-red-800'}`}>
                      <strong>{bulkStatus.type === 'success' ? '✔ Batch Successful:' : '❌ Upload Error:'}</strong>
                ...
                <div className="mt-4 pt-3 border-t border-dashed border-zinc-400">
                  <div className="text-[8px] font-mono leading-snug text-zinc-500 uppercase bg-[#F5F5F3] p-2 border border-[#141414]">
                    💡 CSV Columns must keep titles corresponding precisely to: 
                    <strong className="block text-black mt-1">title, description, listing_type, price_per_month, deposit, address, latitude, longitude, apartment_type, amenities, image_urls</strong>
                  </div>
                </div>
              </aside>
            )}
            ...
            <main className="flex-1 overflow-y-auto p-5 bg-[#E4E3E0] flex flex-col gap-4">
              ...
              <section className="border-2 border-[#141414] bg-white p-5 shadow-[4px_4px_0px_#141414]">
                <div className="flex justify-between items-center mb-4 pb-2 border-b border-[#141414]">
                  <div>
                    <span className="text-[10px] font-mono opacity-50 uppercase tracking-widest block mb-1">Landlord Management Suite</span>
                    <h1 className="text-xl font-black uppercase tracking-tight">Active Real Estate Portfolios</h1>
                  </div>
                  <div className="flex gap-2">
                    <button 
                      ...
                      title={showBulkImporter ? "Hide CSV / JSON bulk uploading options" : "Reveal CSV / JSON bulk uploading options"}
                    >
                      <Upload className="w-3.5 h-3.5 animate-pulse" />
                      {showBulkImporter ? "Hide Bulk Importer" : "Bulk Import Tools"}
                    </button>
                    <button 
                ...
                <div className="mb-2">
                  <span className="text-[9px] font-mono text-zinc-500 uppercase tracking-wider font-extrabold block mb-1">
                    ⚙️ Sim Svc Tiers (Click Card to Alter Active Platform Plan)
                  </span>
                </div>
                <div className="grid grid-cols-4 gap-2">
                  {[
                    { key: 'free', label: 'Free Tier', price: '₹0/mo', desc: 'Max 1 listing' },
                    { key: 'premium_seeker', label: 'Premium Seeker', price: '₹249/mo', desc: 'Boost matches' },
                    { key: 'owner_pro', label: 'Owner Pro', price: '₹1,499/mo', desc: '15 units active' },
                    { key: 'owner_enterprise', label: 'Owner Enterprise', price: '₹4,999/mo', desc: 'Unlimited API' }
                  ].map((tier) => {
                    ...
                    return (
                      ...
                      >
                        <div className="text-[9px] font-mono opacity-60 font-black uppercase">{tier.price}</div>
                        <div className="text-[10.5px] font-extrabold leading-none my-0.5 uppercase tracking-tight">{tier.label}</div>
                        <p className="text-[8px] opacity-75">{tier.desc}</p>
                        {isCurrent && <span className="text-[7.5px] font-mono bg-[#D2F57C] text-black font-black px-1.5 py-0.2 uppercase mt-1 inline-block">Active Plan</span>}
                      </div>
                    );
                  })}
                </div>
              ...
              <section className="border-2 border-[#141414] bg-white shadow-[4px_4px_0px_#141414] flex-1 flex flex-col justify-between">
                <div>
                  <div className="p-3 bg-[#F5F5F3] border-b border-[#141414] text-xs font-black uppercase tracking-wider flex justify-between items-center">
                    <span>Active Live Listings Catalog ({ownerListings.length})</span>
                    <span className="text-[9px] font-mono text-zinc-500 font-normal">Manage portfolios in Bengaluru</span>
                  </div>
                  ...
                  <div className="divide-y divide-[#141414] overflow-y-auto max-h-[290px]">
                    {ownerListings.length === 0 ? (
                      <div className="p-12 text-center text-xs text-zinc-500 italic font-mono">
                        No property assets published under this account.<br/>Use CSV Bulk Importer on left drawer block to load initial mock setups.
                      </div>
                    ) : (
                      ownerListings.map((l) => (
                        <div key={l.id} className="p-3 bg-white hover:bg-neutral-50 grid grid-cols-[1fr_80px_100px_120px] items-center gap-4 text-xs font-mono">
                          <div>
                            <span className="font-sans font-black text-xs text-[#141414] block truncate">{l.title}</span>
                            <span className="text-[9px] opacity-65 truncate block max-w-sm">{l.address} (PIN: {l.pincode || "560038"})</span>
                          </div>
                          <div>
                            <span className="font-extrabold text-[#141414] block uppercase text-[9px]">
                              {l.listing_type === 'shared_stay' ? 'Shared Stay' : 'Entire Unit'}
                            </span>
                            <span className="text-[8.5px] opacity-50">{l.room_size || l.apartment_type || 'Custom'}</span>
                          </div>
                          <div>
                            <span className="font-bold text-green-700 block">{formatPrice(l.price_per_month)}/mo</span>
                            <span className="text-[8.5px] opacity-50">Deposit: {formatPrice(l.deposit)}</span>
                          </div>
                          <div className="flex flex-col gap-1 items-end text-right">
                            {l.is_verified || l.verification_status === 'verified' ? (
                              <span className="text-[8px] bg-green-100 text-green-800 border border-green-400 px-1.5 py-0.5 font-bold uppercase tracking-wide rounded-sm mb-1">✔ Verified Stay</span>
                            ) : l.verification_status === 'pending' ? (
                              <button 
                                onClick={() => handleApproveVerificationSim(l.id)}
              ...
              {currentUser?.user_type === 'owner' ? (
                ...
                <div>
                  <div className="flex justify-between items-center pb-3 border-b border-[#141414] mb-4">
                    <div>
                      <h1 className="text-xl font-black uppercase tracking-tight">Property Policy & Tenant Rules Manager</h1>
                      <p className="text-[9px] font-mono opacity-50 uppercase mt-0.5">
                        Configure target tenant requirements and house expectations for your rental portfolio
                      </p>
                    </div>
                    <div className="text-right">
                      <span className="text-xs font-mono opacity-55">Landlord ID:</span>
                      <span className="block text-xs font-bold font-mono">{currentUser?.id}</span>
                    </div>
                  </div>
                  ...
                  <form onSubmit={handleSaveProfile} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-wider mb-1 font-mono text-zinc-700">Landlord Representative Name</label>
                        <input 
                          type="text"
                      ...
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-wider mb-1 font-mono text-zinc-700">Simulated Contact Mobile (shown in stays)</label>
                        <input 
                          type="text"
                    ...
                    <div className="grid grid-cols-3 gap-4 pb-2 border-b border-[#141414] border-dashed">
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-wider mb-1 font-mono text-zinc-700">Years of Property Experience</label>
                        <input 
                          type="number"
                      ...
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-wider mb-1 font-mono text-zinc-700">Target Tenant Gender Cohort</label>
                        <select
                          className="w-full border border-[#141414] p-2 text-xs bg-[#F5F5F3] outline-none rounded-none"
                          value={editGender}
                          onChange={(e) => setEditGender(e.target.value)}
                        >
                          <option value="Not Specified">No Specific Gender Restriction (Co-ed)</option>
                          <option value="Male">Prefer Male Cohort</option>
                          <option value="Female">Prefer Female Cohort</option>
                          <option value="Non-Binary">Prefer Diverse Cohort</option>
                        </select>
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-wider mb-1 font-mono text-zinc-700">Agency / Corporate Registry Brand</label>
                        <input 
                          type="text"
                          className="w-full border border-[#141414] p-2 text-xs bg-[#F5F5F3] outline-none"
                    ...
                    <div>
                      <label className="block text-[10px] font-bold uppercase tracking-wider mb-1 font-mono text-zinc-700">Property Management Greeting & Rules Summary (Visible to Renters)</label>
                      <textarea 
                    ...
                    <div className="bg-indigo-50 p-4 border border-[#141414] space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-xs font-mono">
                        <div>
                          <div className="flex justify-between items-center mb-1">
                            <label className="text-[10px] font-bold uppercase text-zinc-700">Expected Tenant Tidiness Standard</label>
                            <span className="font-extrabold">{editCleanliness} / 5</span>
                          </div>
                          <input 
                            type="range" 
                            min="1" 
                        ...
                        <div>
                          <div className="flex justify-between items-center mb-1">
                            <label className="text-[10px] font-bold uppercase text-zinc-700">Target Monthly Rent Budget Limits Offered</label>
                            <span className="font-sans font-bold text-zinc-800">{formatPrice(editBudgetMin)} - {formatPrice(editBudgetMax)}</span>
                          </div>
                          <div className="flex gap-2">
                            <input 
                              type="number" 
                      ...
                      <div className="grid grid-cols-3 gap-2 text-xs font-mono">
                        <div>
                          <label className="block text-[9px] font-bold uppercase mb-1 text-zinc-600">WFH Lifestyle Support</label>
                          <select 
                            value={editWfh} 
                          ...
                          >
                            <option value="office">Suited primarily for commuter/offline workers</option>
                            <option value="hybrid">Suited easily for Hybrid routines</option>
                            <option value="wfh">Equipped with reliable desks & power backups for permanent WFH</option>
                          </select>
                        </div>

                        <div>
                          <label className="block text-[9px] font-bold uppercase mb-1 text-zinc-600">Quiet Hours Expectation</label>
                          <select 
                            value={editSleeping} 
                            onChange={(e: any) => setEditSleeping(e.target.value)}
                            className="w-full border border-[#141414] bg-white p-1 text-[11px] rounded-none shadow-sm"
                          >
                            <option value="early_bird">Strict early hours quiet zone required</option>
                            <option value="night_owl">Flexible late-hours friendly premise</option>
                            <option value="flexible">Discretionary / No formal limits</option>
                          </select>
                        </div>

                        <div>
                          <label className="block text-[9px] font-bold uppercase mb-1 text-zinc-600">Premise Gathering/Party Policy</label>
                          <select 
                            value={editDrinking} 
                            onChange={(e: any) => setEditDrinking(e.target.value)}
                            className="w-full border border-[#141414] bg-white p-1 text-[11px] rounded-none shadow-sm"
                          >
                            <option value="never">Parties or substance gatherings strictly prohibited</option>
                            <option value="socially">Social guests and occasional meetups permitted</option>
                            <option value="regularly">Flexible social guidelines</option>
                          </select>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 pt-1">
                        ...
                        <label className="flex items-center gap-2 cursor-pointer text-xs select-none font-mono">
                          <input 
                            ...
                            onChange={(e) => setEditPetsAllowed(e.target.checked)}
                            className="w-4 h-4 accent-[#141414] border border-[#141414]"
                          />
                    ...
                    <div className="flex justify-between items-center gap-4 pt-2">
                      {profileSaveStatus === 'updating' ? (
                        <span className="text-xs font-mono opacity-70 flex items-center gap-1">
                          <Loader2 className="w-4 h-4 animate-spin text-zinc-900" /> Save changes operating...
                        </span>
                      ) : profileSaveStatus === 'success' ? (
                        <span className="text-xs font-mono text-green-700 font-bold">✓ Property Management Criteria successfully synchronized!</span>
                      ) : profileSaveStatus === 'error' ? (
                        <span className="text-xs font-mono text-red-650 font-bold">❌ Save failed. Verify parameter constraints.</span>
                      ) : <div />}
                ...
                <div>
                  <div className="flex justify-between items-center pb-3 border-b border-[#141414] mb-4">
                    <div>
                      <h1 className="text-xl font-black uppercase tracking-tight">Preferences Matrix Settings</h1>
                      <p className="text-[9px] font-mono opacity-50 uppercase mt-0.5">
                        Modifying this matrix recalibrates the roommate matching score instantly
                      </p>
                    </div>
                    <div className="text-right">
                      <span className="text-xs font-mono opacity-55">Account ID:</span>
                      <span className="block text-xs font-bold font-mono">{currentUser?.id}</span>
                    </div>
                  </div>
                  ...
                  <form onSubmit={handleSaveProfile} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-wider mb-1 font-mono">Full Name</label>
                        <input 
                          type="text"
                          className="w-full border border-[#141414] p-2 text-xs bg-[#F5F5F3] outline-none"
                          value={editFullName}
                      ...
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-wider mb-1 font-mono">Simulated Mobile (hidden inside platform)</label>
                        <input 
                          type="text"
                          className="w-full border border-[#141414] p-2 text-xs bg-[#F5F5F3] outline-none"
                          value={editPhone}
                    ...
                    <div className="grid grid-cols-3 gap-4 pb-2 border-b border-[#141414] border-dashed">
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-wider mb-1 font-mono">Age (Years)</label>
                        <input 
                          type="number"
                          className="w-full border border-[#141414] p-2 text-xs bg-[#F5F5F3] outline-none"
                          value={editAge}
                      ...
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-wider mb-1 font-mono">Gender Identification</label>
                        <select
                          className="w-full border border-[#141414] p-2 text-xs bg-[#F5F5F3] outline-none rounded-none"
                          value={editGender}
                          onChange={(e) => setEditGender(e.target.value)}
                      ...
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-wider mb-1 font-mono">Active Profession / Occupation</label>
                        <input 
                          type="text"
                          className="w-full border border-[#141414] p-2 text-xs bg-[#F5F5F3] outline-none"
                          value={editProfession}
                    ...
                    {/* Profile bio */}
                    <div>
                      <label className="block text-[10px] font-bold uppercase tracking-wider mb-1 font-mono">Personal Introduction Bio (displayed to matches)</label>
                      <textarea 
                        className="w-full border border-[#141414] p-2.5 text-xs bg-[#F5F5F3] h-20 outline-none font-serif leading-relaxed"
                        value={editBio}
                        onChange={(e) => setEditBio(e.target.value)}
                    ...
                    <div className="bg-[#D2F57C]/10 p-4 border border-[#141414] space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-xs font-mono">
                        <div>
                          <div className="flex justify-between items-center mb-1">
                            <label className="text-[10px] font-bold uppercase">Hygiene & Cleanliness Level</label>
                            <span className="font-extrabold">{editCleanliness} / 5</span>
                          </div>
                          <input 
                            type="range" 
                        ...
                        <div>
                          <div className="flex justify-between items-center mb-1">
                            <label className="text-[10px] font-bold uppercase">Budget Limits (Monthly)</label>
                            <span className="font-sans font-bold">{formatPrice(editBudgetMin)} - {formatPrice(editBudgetMax)}</span>
                          </div>
                          <div className="flex gap-2">
                            <input 
                              type="number" 
                      ...
                      <div className="grid grid-cols-3 gap-2 text-xs font-mono">
                        <div>
                          <label className="block text-[9px] font-bold uppercase mb-1">Workspace Style</label>
                          <select 
                            value={editWfh} 
                            onChange={(e: any) => setEditWfh(e.target.value)}
                            className="w-full border border-[#141414] bg-white p-1 text-[11px] rounded-none shadow-sm"
                        ...
                        <div>
                          <label className="block text-[9px] font-bold uppercase mb-1">Sleeping Cycle</label>
                          <select 
                            value={editSleeping} 
                            onChange={(e: any) => setEditSleeping(e.target.value)}
                            className="w-full border border-[#141414] bg-white p-1 text-[11px] rounded-none shadow-sm"
                          >
                            ...
                            <option value="flexible">Flexible biological</option>
                          </select>
                        </div>

                        <div>
                          <label className="block text-[9px] font-bold uppercase mb-1">Social Drinking</label>
                          <select 
                            value={editDrinking} 
                            onChange={(e: any) => setEditDrinking(e.target.value)}
                            className="w-full border border-[#141414] bg-white p-1 text-[11px] rounded-none shadow-sm"
                    ...
                    <div className="flex justify-between items-center gap-4">
                      {profileSaveStatus === 'updating' ? (
                        ...
                        <span className="text-xs font-mono text-green-700 font-bold">✓ Preferences successfully synchronized across server!</span>
                      ) : profileSaveStatus === 'error' ? (
                        <span className="text-xs font-mono text-red-650 font-bold">❌ Save failed. Verify parameter constraints.</span>
                      ) : <div />}
      ...
      <footer className="h-8 border-t border-[#141414] bg-white flex items-center justify-between px-4 text-[9px] font-mono uppercase opacity-55 shrink-0 z-10 select-none">
        <div>
          Development Node: Ready // Active User UUID: <span className="font-bold underline">{currentUser?.id || 'seeker-1'}</span>
        </div>
        <div>
        ...
        <div className="absolute inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white border-4 border-[#141414] max-w-sm w-full p-6 shadow-[8px_8px_0px_#141414] relative animate-in fade-in zoom-in duration-150">
            ...
            <div className="pb-2 border-b border-[#141414] mb-4">
              <h3 className="font-serif italic text-sm font-bold uppercase tracking-tight">Onboard Simulation Member</h3>
              <p className="text-[8.5px] font-mono opacity-50 uppercase">Initialize brand new profile context logs</p>
            </div>

            <form onSubmit={handleRegisterUser} className="space-y-3.5 text-xs">
              <div>
                <label className="block text-[9px] font-mono font-bold uppercase mb-1">Full Member Name</label>
                <input 
                  type="text" 
                  value={regName}
              ...
              <div>
                <label className="block text-[9px] font-mono font-bold uppercase mb-1">Email Address</label>
                <input 
                  type="email" 
                  value={regEmail}
              ...
              <div>
                <label className="block text-[9px] font-mono font-bold uppercase mb-1">Desired User Account Role</label>
                <div className="grid grid-cols-3 gap-1">
                  {[
              ...
              <div>
                <label className="block text-[9px] font-mono font-bold uppercase mb-1">Phone Number</label>
                <input 
                  type="text" 
                  value={regPhone}
      ...
      {showNewListingModal && (
        <div className="absolute inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white border-4 border-[#141414] max-w-lg w-full p-5 shadow-[8px_8px_0px_#141414] relative max-h-[660px] overflow-y-auto">
            ...
            <div className="pb-2 border-b border-[#141414] mb-3">
              <h3 className="font-serif italic text-sm font-bold uppercase tracking-tight">Create Stay Listing Record</h3>
              <p className="text-[8.5px] font-mono opacity-50 uppercase">Publish rent assets directly into FlatMatch Directory</p>
            </div>

            <form onSubmit={handleCreateListing} className="space-y-3.5 text-xs">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[9px] font-mono font-bold mb-1">Listing Category</label>
                  <select 
                  ...
                  >
                    <option value="shared_stay">Shared Stay (Existing Tenant room)</option>
                    <option value="entire_unit">Entire vacant unit (Landlord/Owner)</option>
                  </select>
                </div>
                <div>
                  <label className="block text-[9px] font-mono font-bold mb-1">Rent Title headings</label>
                  <input 
                    type="text" 
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-[9px] font-mono font-bold mb-1">Rent share (₹/month)</label>
                  <input 
                    type="number" 
                ...
                <div>
                  <label className="block text-[9px] font-mono font-bold mb-1">Lease Deposit (₹)</label>
                  <input 
                    type="number" 
                ...
                <div>
                  <label className="block text-[9px] font-mono font-bold mb-1">Location street Address</label>
                  <input 
                    type="text" 
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-[9px] font-mono font-bold mb-1">City</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="block text-[9px] font-mono font-bold mb-1">State</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="block text-[9px] font-mono font-bold mb-1">PIN Code (6 digits)</label>
                  <input 
                    type="text" 
              ...
              {newListingType === 'shared_stay' ? (
                <div className="grid grid-cols-3 gap-3 p-3 bg-neutral-100 border border-[#141414]">
                  <div>
                    <label className="block text-[9px] font-mono font-bold mb-1">Room Size</label>
                    <input 
                      type="text" 
                  ...
                  <div>
                    <label className="block text-[9px] font-mono font-bold mb-1">Utility split terms</label>
                    <input 
                      type="text" 
                  ...
                  <div>
                    <label className="block text-[9px] font-mono font-bold mb-1">Existing Flatmates</label>
                    <input 
                      type="number" 
                ...
                <div className="p-3 bg-neutral-100 border border-[#141414]">
                  <label className="block text-[9px] font-mono font-bold mb-1">Apartment Layout Type</label>
                  <select 
              ...
              <div>
                <label className="block text-[9px] font-mono font-bold mb-1">Included Amenities (comma separated)</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="block text-[9px] font-mono font-bold mb-1">Cover Image scene photo URL</label>
                <input 
                  type="text" 
                  value={newListingImg}
              ...
              <div className="grid grid-cols-2 gap-4 p-2.5 bg-red-50/70 border-2 border-red-400">
                <div>
                  <label className="block text-[9.5px] font-mono font-bold mb-1 text-red-800">Roommate Gender Pref *</label>
                  <select 
                ...
                <div>
                  <label className="block text-[9.5px] font-mono font-bold mb-1 text-red-800">Mandatory House Rules / Restrictions *</label>
                  <input 
                    type="text" 
                    ...
                    required
                  />
                  <p className="text-[7.5px] text-red-600 mt-1 font-mono">Minimum 1 limit required; separate by comma</p>
                </div>
              </div>

              <div>
                <label className="block text-[9px] font-mono font-bold mb-1">Description</label>
                <textarea 
        ...
        <div className="absolute inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white border-4 border-red-600 max-w-md w-full p-5 shadow-[8px_8px_0px_rgba(220,38,38,1)] relative">
            ...
            <div className="pb-2 border-b-2 border-red-500 mb-3 flex items-center gap-2">
              ...
              <div>
                <h3 className="font-serif italic text-xs font-bold uppercase tracking-tight text-red-700">Flag Inappropriate Contact</h3>
                <p className="text-[8.5px] font-mono opacity-60 uppercase">FlatMatch Trust & Platonic Safety Guardianship</p>
              </div>
            </div>

            <form onSubmit={handleSendReport} className="space-y-3.5 text-xs text-[#141414]">
              <div>
                <label className="block text-[9px] font-mono font-bold mb-1">User being reported</label>
              ...
              <div>
                <label className="block text-[9px] font-mono font-bold mb-1">Category of concern</label>
                <select 
                  value={reportReason} 
                ...
                >
                  <option value="Romantic solicitation / dating-like behavior">Romantic solicitation / Dating-like behavior</option>
                  <option value="Inappropriate flirting or pickup lines">Inappropriate flirting or pickup lines</option>
                  <option value="Harassment or offensive language">Harassment or offensive language</option>
                  <option value="Spam or fake listings / scam attempt">Spam or fake listings / scam attempt</option>
                </select>
              </div>

              <div>
                <label className="block text-[9px] font-mono font-bold mb-1">Specific details or message text evidence</label>
                <textarea 
                  value={reportDetails}
              ...
              <div className="p-2.5 bg-red-50 text-[10px] text-red-800 border border-red-350 leading-snug font-medium">
                <strong>Important:</strong> FlatMatch is strictly platonic. Sending false claims is also a violation. Safe spaces are maintained by objective audits. Thank you for reporting!
              </div>
